...
class Loader
{
    public function __construct()
    {
        ...
        // We looking for available command on local instance. This assume of course that local and remote instances
        // have the same version of TYPO3 and extensions which should be true for most cases.
        $deployerTasksGroups = [];
                ...
                foreach ($deployerTasksGroup['commands'] as $commandRawLine) {
                    ...
                    if (is_array($match) && isset($match[1]) && isset($match[2])) {
                        ...
                        if (preg_match('/^[a-zA-Z:]+$/', $taskKey)) {
                            ...
                            \Deployer\task($deployerTasksGroup['deployerPrefix'] . ':' . $taskKey,
                                function () use ($taskKey, $deployerTasksGroupBinary) {
                                    $option = '';
                                    ...
                                    {{bin/php}} ' . $command . ' ' . $taskKey . ' ' . $option);
